...
### 4. 커밋 전에 **인덱스**를 본다 — 워킹트리가 아니라

```bash
git diff --cached --stat    # 커밋 직전 필수
```

`git status`(워킹트리)만 보고 커밋하지 않는다. **앞서 `git rm`·`git add` 로 스테이징해둔
것이 남아 있으면 의도하지 않은 파일이 같이 담긴다.** 실제로 그렇게 앱 코드 삭제가
콘텐츠 커밋에 섞여 배포 금지 상태에서 푸시된 적이 있다.

**앱 코드 변경이 섞이면 커밋을 분리한다.** 콘텐츠·문서와 앱 코드는 배포 게이트가
다르므로 한 커밋에 담으면 한쪽만 푸시할 수 없다.

### 5. `content-factory/progress.md` 는 공용 — append-only
