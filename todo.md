...
## Phase 10: 지원하기 기능
- [x] job_applications 테이블 DB 스키마 추가
- [x] jobApplications tRPC 라우터 구현 (submit, withdraw, myApplications, jobApplications)
- [x] JobDetail 페이지에 지원하기 버튼 및 지원 모달 추가
- [x] 마이페이지에 내 지원 현황 탭 추가
- [x] 살롱 원장 지원자 목록 확인 기능 (JobDetail 내)
- [x] 살롱 원장 지원자 상태 변경 기능 (已查看/錄取/不合適/重設)

## Phase 11: 관리자 대시보드
- [x] DB 관리자 통계 쿼리 함수 추가 (회원, 공고, 지원, 게시글, 양도, 중고)
- [x] adminRouter tRPC 프로시저 추가 (admin role 전용)
- [x] AdminDashboard 페이지 구현 (통계 카드, 차트, 최근 활동)
- [x] /admin 라우트 등록 및 관리자 권한 가드 추가
