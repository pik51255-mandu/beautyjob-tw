...
// 연도는 문자열에 직접 쓰지 말고 {year}(서기)·{rocYear}(민국) 자리표시자를 쓴다.
// makeT 가 insuranceTables2026 의 meta 에서 채운다 — 표를 갈아끼우면 화면이 따라온다.
// 기본 언어 zh-TW, ?lang=ko 일 때만 한국어 (QA용, SEO 미노출).

import { RATE_ROC_YEAR, RATE_YEAR } from "./insuranceTables2026";
...
export const toolStrings = {
  ...
  metaDescription: {
    zh: "台灣美髮設計師・助理專用薪水試算器。輸入業績與抽成率，自動計算勞保、健保、勞退自提與實領金額（{year}年費率）。",
    ko: "대만 미용 디자이너·어시스턴트 전용 급여 계산기. 업적·커미션 입력 시 노보·건보·노퇴 공제와 실수령액 자동 계산 ({year}년 요율).",
  },
  heading: { zh: "薪水試算器", ko: "급여 계산기" },
  subheading: {
    zh: "{year} 年費率｜輸入您的條件，估算每月實領金額",
    ko: "{year}년 요율｜조건을 입력하면 월 실수령액을 추정합니다",
  },
  ...
  ratesNote: {
    zh: "費率基準：{year} 年（民國 {rocYear} 年）公告。最低工資 29,500／勞保 12.5%／健保 5.17%",
    ko: "요율 기준: {year}년(민국 {rocYear}년) 공고. 최저임금 29,500／노보 12.5%／건보 5.17%",
  },
  ...
  ocMetaDescription: {
    zh: "台灣美髮開店成本試算：初期投資（押金・頂讓費・裝潢・設備）與每月固定成本（含雇主勞健保・勞退負擔）、建議準備資金，開店前一次算清楚（{year} 年費率）。",
    ko: "대만 미용실 창업비용 계산: 초기 투자(보증금·권리금·인테리어·설비)와 월 고정비(고용주 노건보·노퇴 부담 포함), 권장 준비 자금까지 창업 전 한 번에 ({year}년 요율).",
  },
  ...
  hubMetaDescription: {
    zh: "台灣美髮人專用工具箱：薪水試算器、開店成本試算器，以及老闆專區的經營工具（{year} 年費率）。",
    ko: "대만 미용인 전용 도구함: 급여 계산기, 창업비용 계산기, 그리고 사장님 전용 경영 도구 ({year}년 요율).",
  },
  ...
  bsMetaDescription: {
    zh: "美髮沙龍經營模擬：服務組合、抽成與手續費、人力產能，計算損益兩平營業額與情境損益（{year} 年費率）。",
    ko: "미용실 경영 시뮬레이션: 시술 믹스·커미션·수수료·인력 캐파로 손익분기 매출과 시나리오 손익 계산 ({year}년 요율).",
  },
  bsSubheading: {
...
export function makeT(lang: ToolLang) {
  return (key: ToolStringKey): string =>
    toolStrings[key][lang]
      .replaceAll("{year}", String(RATE_YEAR))
      .replaceAll("{rocYear}", String(RATE_ROC_YEAR));
}
